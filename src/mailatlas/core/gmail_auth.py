...
def load_valid_gmail_access_token(*, store: FileTokenStore | None = None) -> str:
    store = store or FileTokenStore()
    token = store.load()
    if not token:
    ...
    *,
    ...
    if not token:
        return GmailAuthResult(
            ...
            store_path=store.path.as_posix(),
            email=None,
            scopes=(),
            expires_at=None,
        )
...
def gmail_auth_status(*, store: FileTokenStore | None = None) -> GmailAuthResult:
    store = store or FileTokenStore()
    return token_status(store.load(), store=store)


def gmail_auth_logout(*, store: FileTokenStore | None = None) -> GmailAuthResult:
    store = store or FileTokenStore()
